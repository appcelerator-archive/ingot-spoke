		...
		async.whilst(
			...
			function (next) {
				jobProcessor.getJob(getUrl, {
					...
					// run the job!
					jobProcessor.runJob(job, function (err, results) {
						if (err) {
							ic.printError(err, 'spoke: get job error:');
						}
...
Spoke.prototype.registerJobHandler = function registerJobHandler(name, callback) {
	this.jobHandlers[name] = callback;
};