var appc = require('node-appc'),
	...
	processor = require('./lib/job-processor');
...
exports.start = function start(cfg, callback) {
	...
	var hub = cfg.spoke.hub = cfg.spoke.hub || 'http://localhost:8080',
		capabilities,
		machineId;
	...
	async.series([
		function detectCapabilities(next) {
		...
		function detectMachineId(next) {
			// ioreg -rd1 -c IOPlatformExpertDevice
			// OR
			// /usr/sbin/system_profiler SPHardwareDataType | grep "Hardware UUID" | awk '{print $3}'
			machineId = 'CBC33EE3-26BE-59ED-9204-D0118BD1E4E4';
			next();
		},

		function registerWithHub(next) {
		...
		function processJobs(next) {
			process.nextTick(function () {
				processor.run(appc.util.mix({}, cfg, {
					machineId: machineId,
					types: ['test']
				}));
			});
